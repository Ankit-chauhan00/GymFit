...
import { getUserForTrainer } from "@/lib/actions/trainer.action";
...
const AddClient = async ({ searchParams }: SearchParams) => {
  const params = await searchParams;
  const { page, pageSize, query, filter } = params;

  const { data } = await getUserForTrainer({
    page: Number(page) || 1,
        ...
        <header className="flex flex-col">
          ...
          <div className="mt-2 flex flex-col md:mt-5">
            <div className="mt-5 flex flex-col gap-4 md:flex-row">
              <LoaclSearch
                route={`${ROUTES.TRAINER_PANNEL}/add-clients`}
                iconPosition="right"
